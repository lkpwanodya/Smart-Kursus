              ...
              <div className="fixed inset-0 z-50 flex justify-center items-start overflow-y-auto p-4 bg-neutral-950/65 backdrop-blur-md animate-fade-in text-left">
                ...
                <div className="relative bg-white rounded-3xl border border-neutral-100 shadow-2xl w-full max-w-md overflow-hidden z-10 transition-transform transform scale-100 p-6 md:p-8 space-y-5 my-auto">
                  ...
                  <div className="mt-4 pt-4 border-t border-neutral-100 space-y-2 text-left">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-neutral-450 flex items-center gap-1">
                      <Sparkles className="w-3 h-3 text-emerald-600" />
                      <span>💡 Akun Demo Uji Coba (Klik untuk Mengisi)</span>
                    </p>
                    <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1 scrollbar-thin">
                      ...
                      {institutions.slice(0, 3).map((inst) => (
                        <div 
                          key={inst.id}
                          onClick={() => {
                            setLoginEmail(inst.email);
                            setLoginPassword(inst.password);
                          }}
                          className="p-2 border border-dashed border-neutral-200 hover:border-emerald-500 rounded-xl bg-neutral-50 hover:bg-emerald-50/30 cursor-pointer transition-all text-xs"
                        >
                          <div className="font-bold text-neutral-800 truncate flex justify-between items-center">
                            <span>{inst.name}</span>
                            <span className="text-[9px] bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded-md font-mono">Pimpinan</span>
                          </div>
                          <div className="text-neutral-500 flex flex-col sm:flex-row sm:justify-between gap-1 mt-1 font-mono text-[10px]">
                            <span className="truncate">Email: <strong>{inst.email}</strong></span>
                            <span className="text-emerald-700 font-bold shrink-0">Sandi: {inst.password}</span>
                          </div>
                        </div>
                      ))}
                      ...
                      <div 
                        onClick={() => {
                          setLoginEmail('superadmin');
                          setLoginPassword('superadmin123');
                        }}
                        className="p-2 border border-dashed border-neutral-200 hover:border-purple-500 rounded-xl bg-neutral-50 hover:bg-purple-50/30 cursor-pointer transition-all text-xs"
                      >
                        <div className="font-bold text-neutral-800 flex justify-between items-center">
                          <span>👑 Superadmin (Pengawas SaaS)</span>
                          <span className="text-[9px] bg-purple-50 text-purple-700 px-1.5 py-0.5 rounded-md font-mono">Global</span>
                        </div>
                        <div className="text-neutral-500 flex justify-between gap-2 mt-1 font-mono text-[10px]">
                          <span>User: <strong>superadmin</strong></span>
                          <span className="text-purple-700 font-bold">Sandi: superadmin123</span>
                        </div>
                      </div>

                      {/* Staff accounts helper for first loaded institution */}
                      {institutions.length > 0 && institutions[0].staffCredentials && institutions[0].staffCredentials.length > 0 && (
                        <div className="pt-2 border-t border-neutral-100 space-y-1">
                          <p className="text-[9px] font-bold text-neutral-400 uppercase tracking-wider">Kredensial Demo Peran Staf ({institutions[0].name}):</p>
                          {institutions[0].staffCredentials.slice(0, 3).map((staff) => (
                            <div 
                              key={staff.id}
                              onClick={() => {
                                setLoginEmail(staff.username);
                                setLoginPassword(staff.password);
                              }}
                              className="p-1.5 border border-neutral-150 rounded-lg hover:border-indigo-500 bg-neutral-50/50 hover:bg-indigo-50/10 cursor-pointer transition-all text-[10px]"
                            >
                              <div className="flex justify-between font-medium text-neutral-700">
                                <span>{staff.name}</span>
                                <span className="text-[8px] bg-indigo-50 text-indigo-700 px-1 py-0.5 rounded capitalize">{staff.role.replace('_', ' ')}</span>
                              </div>
                              <div className="text-[9px] text-neutral-500 flex justify-between mt-0.5 font-mono">
                                <span>User: <strong>{staff.username}</strong></span>
                                <span className="text-indigo-600 font-bold">Sandi: {staff.password}</span>
                              </div>
                            </div>
                          ))}
                        </div>
