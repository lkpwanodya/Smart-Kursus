            ...
            {showLoginModal && (
              <div className="fixed inset-0 z-50 flex justify-center items-start overflow-y-auto p-4 bg-neutral-950/65 backdrop-blur-md animate-fade-in text-left">
                ...
                <div className="relative bg-white rounded-3xl border border-neutral-100 shadow-2xl w-full max-w-md overflow-hidden z-10 transition-transform transform scale-100 p-6 md:p-8 space-y-5 my-auto">
                  ...
                  {/* Sandbox Developer Shortcut Credentials */}
                  <div className="bg-emerald-50/50 border border-emerald-100/80 rounded-2xl p-3.5 space-y-2">
                    <span className="text-[10px] sm:text-[11px] font-bold text-emerald-800 flex items-center gap-1.5 uppercase tracking-wider">
                      <BrainCircuit className="w-3.5 h-3.5 animate-pulse text-emerald-600" />
                      Pintas Kredensial (Dev Mode)
                    </span>
                    
                    {/* Role Headers & Groups */}
                    <div className="space-y-2">
                      <div>
                        <p className="text-[8px] font-black uppercase text-neutral-400 mb-1">Akses Utama Pimpinan</p>
                        <div className="grid grid-cols-2 gap-1.5">
                          <button
                            type="button"
                            onClick={() => {
                              setLoginEmail('tech@lkp.id');
                              setLoginPassword('password123');
                              setLoginError('');
                            }}
                            className="text-left p-2 rounded-xl bg-white hover:bg-neutral-50 border border-neutral-200 hover:border-emerald-500 shadow-4xs transition-all cursor-pointer group"
                          >
                            <p className="text-[10px] font-extrabold text-neutral-800 uppercase leading-none mb-0.5">Computer Tech</p>
                            <p className="text-[8px] font-mono text-neutral-450 truncate">tech@lkp.id / password123</p>
                          </button>
                          ...
                          <button
                            ...
                            className="text-left p-2 rounded-xl bg-white hover:bg-neutral-50 border border-neutral-200 hover:border-emerald-500 shadow-4xs transition-all cursor-pointer group"
                          >
                            <p className="text-[10px] font-extrabold text-neutral-800 uppercase leading-none mb-0.5">Superadmin</p>
                            <p className="text-[8px] font-mono text-neutral-450 truncate">superadmin / superadmin123</p>
                          </button>
                        </div>
                      </div>

                      <div>
                        <p className="text-[8px] font-black uppercase text-neutral-400 mb-1">Akses Staff Lembaga Kursus (Hasil Pengawasan Pimpinan)</p>
                        <div className="grid grid-cols-3 gap-1.5">
                          <button
                            type="button"
                            onClick={() => {
                              setLoginEmail('admin@wanodya.lkp');
                              setLoginPassword('password123');
                              setLoginError('');
                            }}
                            className="text-left p-2 rounded-xl bg-white hover:bg-neutral-50 border border-neutral-200 hover:border-emerald-500 shadow-4xs transition-all cursor-pointer group"
                          >
                            <p className="text-[9px] font-extrabold text-neutral-800 uppercase leading-none mb-0.5">💼 Staf Admin</p>
                            <p className="text-[7px] font-mono text-neutral-450 truncate">admin@wanodya.lkp</p>
                          </button>

                          <button
                            type="button"
                            onClick={() => {
                              setLoginEmail('bendahara@wanodya.lkp');
                              setLoginPassword('password123');
                              setLoginError('');
                            }}
                            className="text-left p-2 rounded-xl bg-white hover:bg-neutral-50 border border-neutral-200 hover:border-emerald-500 shadow-4xs transition-all cursor-pointer group"
                          >
                            <p className="text-[9px] font-extrabold text-neutral-800 uppercase leading-none mb-0.5">💰 Bendahara</p>
                            <p className="text-[7px] font-mono text-neutral-450 truncate">bendahara@wanodya.lkp</p>
                          </button>

                          <button
                            type="button"
                            onClick={() => {
                              setLoginEmail('budi@wanodya.lkp');
                              setLoginPassword('password123');
                              setLoginError('');
                            }}
                            className="text-left p-2 rounded-xl bg-white hover:bg-neutral-50 border border-neutral-200 hover:border-emerald-500 shadow-4xs transition-all cursor-pointer group"
                          >
                            <p className="text-[9px] font-extrabold text-neutral-800 uppercase leading-none mb-0.5">🎓 Pengajar</p>
                            <p className="text-[7px] font-mono text-neutral-450 truncate">budi@wanodya.lkp</p>
                          </button>
                        </div>
                      </div>
                    </div>

                    <p className="text-[9px] text-neutral-450 leading-tight pt-1">
                      * Klik tombol di atas untuk mengisi formulir login instan dan menguji hak akses masing-masing peran secara real-time.
                    </p>
                  </div>
